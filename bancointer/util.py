...
class Util(object):
    ...
    def make_request_with_token(self, method, path, json, **kwargs):
        self.add_header_authorization(self._get_token()['access_token'])
        return self.make_request(method=method, path=path, json=json, cert=self.cert, **kwargs)

    def make_request(self, method, path, json, **kwargs):
    ...
    def _get_api_token(self):
        ...
        response = requests.post(self.base_url_token, data=payload, headers=headers, cert=self.cert)
    ...
    def file_save(self, response, file_path):
