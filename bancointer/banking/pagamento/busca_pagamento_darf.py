...
class BuscaPagamentoDarf(object):
    ...
    def buscar(self, query_params: dict) -> RespostaBuscaPagamentoDarf | dict:
        """Metodo para obter informações de pagamento de DARF. Escopo requerido: `pagamento-boleto.read`
        ...
        try:
            ...
            path = f"{PATH_PAGAMENTO}/darf?{url_params}"
            ...
            return RespostaBuscaPagamentoDarf(**response).to_dict()
        except ErroApi as e:
            print(f"ErroApi: {e.title}: {e.detail} - violacoes: {e.violacoes}")
            return e.to_dict()
        except BancoInterException as e:
            print(f"BancoInterException.BuscaPagamentoDarf.buscar: {e}")
            return e.erro.to_dict()
        except Exception as e:
            print(f"Exception.BuscaPagamentoDarf: {e}")
