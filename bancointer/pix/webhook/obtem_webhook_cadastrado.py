...
class ObtemWebhookCadastrado(object):
    def __init__(
        self,
    ...
    def obter(self, chave: str) -> dict | ErroApi:
        """Metodo para obter as informações do webhook de acordo com a chave informada.
        Escopo requerido: webhook.read

        Returns:
            dict | ErroApi: Com as informações do webhook(webhookUrl, chave, criacao).
        """
        try:
            # validations
            if not chave or chave is None:
                erro = Erro(
                    404,
                    f"O atributo 'obtemWebhookCadastrado.chave' é obrigatório.",
                )
            ...
            response = self.http_util.make_get(f"{PATH_PIX_WEBHOOK}/{chave}")
            ...
            print(f"BancoInterException.ObtemWebhookCadastrado.obter: {e}")
            return e.erro.to_dict()
        except Exception as e:
            print(f"Exception.ObtemWebhookCadastrado: {e}")
